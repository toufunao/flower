...
def load_data(path, dataset="cora"):
    ...
    features = torch.tensor(np.array(idx_features_labels[:, 1:-1], dtype=np.float32), dtype=torch.float)
    encode_label = LabelEncoder()
    labels = encode_label.fit_transform(idx_features_labels[:, -1])
    # build graph
    idx = np.array(idx_features_labels[:, 0], dtype=np.int32)
    idx_map = {j: i for i, j in enumerate(idx)}
    old_edges_unordered = np.genfromtxt("{}{}.cites".format(path, dataset), dtype=np.long)
    edges_unordered = []
    ...
    edges = edges.T
    new_edges = torch.tensor(edges, dtype=torch.long)

    node_list = [idx_map[item[0].astype(np.int32)] for item in idx_features_labels]
    idx_train, idx_val, idx_test = split(node_list, shuffle=True, ratio_1=0.2, ratio_2=0.4)
    ...
    return new_edges, features, labels, train_mask, val_mask, test_mask
