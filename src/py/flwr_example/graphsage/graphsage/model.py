...
def load_cora():
    num_nodes = len(pd.read_table("cora1/cora.content",header=None))
    num_feats = 1433
    feat_data = np.zeros((num_nodes, num_feats))
    labels = np.empty((num_nodes, 1), dtype=np.int64)
    node_map = {}
    label_map = {}
    with open("cora1/cora.content") as fp:
    ...
    with open("cora1/cora.cites") as fp:
        for i, line in enumerate(fp):
            info = line.strip().split()
...
def run_pubmed():
    ...
    val_output = graphsage.forward(val)
    print
    "Validation F1:", f1_score(labels[val], val_output.data.numpy().argmax(axis=1), average="micro")
    print
    "Average batch time:", np.mean(times)
